...
// Controls component for a mixer interface
function Controls({ controls, setControls, onSave, onLoad }) {
    ...
    const handleChange = (e) => {
        ...
        // Try to parse the value as a number, otherwise keep it as string
        const maybeNum = Number(value);
        setControls(prev => ({
            ...prev,
            [name]: Number.isNaN(maybeNum) ? value : maybeNum
        }));
    };

    // Toggle solo mode for a specific channel
    // If enabling solo on one channel, turn off solo for the others
    ...
    return (
        <div className="mixer-controls">
            ...
            <div className="mb-3">
                ...
                {/* Dropdown to select drum pattern */}
                <Form.Select
                    name="drum_pattern"
            ...
            <div className="form-check form-switch mb-3">
                <input
                    ...
                    onChange={handleChange}
                />
                <label className="form-check-label" htmlFor="showDrums2">
                    Show second drum layer
                </label>
            </div>
        </div>
    );
